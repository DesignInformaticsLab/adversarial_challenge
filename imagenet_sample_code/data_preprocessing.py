...
inf = open(os.path.join(imagenet_dir,'label_to_id.txt'))
inf = inf.read()
inf = re.findall(r"n+\d+", inf)
for iii in range(len(inf)):
    label = inf[iii]
    name_to_id[label] = iii
    id_to_name[iii] = label




for _, name_i in enumerate(os.listdir(train_dir)):
    i = name_to_id[name_i]
